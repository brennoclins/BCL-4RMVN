...
export function AudioScreen({ trackName, artistName, isPlaying }: AudioScreenProps) {
  ...
  return (
    <section className="w-full min-w-0 bg-[#0a0a0a] rounded-xl p-6! shadow-[inset_0_0_20px_rgba(0,0,0,0.8)] border-[3px] border-[#2a2a2a] font-mono min-h-45 flex flex-col justify-between overflow-hidden">
      <div className="min-w-0">
        <div className="text-[#008822] text-[0.8rem] uppercase truncate" id="artistName">{artistName}</div>
        <div className="text-accent-screen text-[1.4rem] uppercase truncate whitespace-nowrap overflow-hidden text-ellipsis" id="trackName">
          {trackName}
        </div>
      </div>
      <div
